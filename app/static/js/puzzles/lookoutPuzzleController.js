...
class LookoutPuzzleController {
  ...
  initialize() {
    ...
    if (puzzleType === "lookout_puzzle_1") {
      this._setupSurveillancePuzzle(gameArea);
    } else if (puzzleType === "lookout_puzzle_2") {
      this._setupPatrolPatternPuzzle(gameArea);
    } else if (puzzleType === "lookout_puzzle_3") {
      this._setupSecuritySystemPuzzle(gameArea);
    } else if (puzzleType === "lookout_puzzle_4") {
      this._setupAlarmPuzzle(gameArea);
    } else if (puzzleType === "lookout_puzzle_5") {
      this._setupEscapeRoutePuzzle(gameArea);
    } else {
      // Fallback to surveillance puzzle
      this._setupSurveillancePuzzle(gameArea);
    }
  ...
  handleRandomEvent(eventType, duration) {
    ...
    // For the Lookout role, we'll make random events less disruptive (since they're supposed to be good at handling them)
    setTimeout(() => {
      this.messageElement.className = "mb-4 text-yellow-400 text-center hidden";
  ...
  /**
   * Private: Setup surveillance puzzle (Stage 1)
  ...
  /**
   * Private: Setup Patrol Pattern puzzle (Stage 2)
   * Would be implemented for stage 2
   */
  _setupPatrolPatternPuzzle(container) {
    const message = document.createElement("div");
    message.className = "text-center text-green-400";
    message.textContent = "Patrol pattern puzzle will be available in Stage 2";
    container.appendChild(message);
  }

  /**
   * Private: Setup Security System puzzle (Stage 3)
   * Would be implemented for stage 3
   */
  _setupSecuritySystemPuzzle(container) {
    const message = document.createElement("div");
    message.className = "text-center text-green-400";
    message.textContent =
      "Security system mapping puzzle will be available in Stage 3";
    container.appendChild(message);
  }

  /**
   * Private: Setup Alarm puzzle (Stage 4)
   * Would be implemented for stage 4
   */
  _setupAlarmPuzzle(container) {
    const message = document.createElement("div");
    message.className = "text-center text-green-400";
    message.textContent = "Alarm system puzzle will be available in Stage 4";
    container.appendChild(message);
  }

  /**
   * Private: Setup Escape Route puzzle (Stage 5)
   * Would be implemented for stage 5
   */
  _setupEscapeRoutePuzzle(container) {
    const message = document.createElement("div");
    message.className = "text-center text-green-400";
    message.textContent =
      "Escape route planning puzzle will be available in Stage 5";
    container.appendChild(message);
  }
}
